...
export default function ClientDashboard() {
  ...
  return (
    <div className="space-y-8 mt-10 md:mt-0 max-w-7xl mx-auto p-4 md:p-0 font-sans text-[var(--text)] selection:bg-[var(--primary)]/20 selection:text-[var(--text)]">
      {/* হেডার সেকশন */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          ...
          className="inline-flex items-center justify-center gap-2 px-5 py-3 font-bold text-sm text-white bg-[var(--primary)] hover:bg-[var(--accent)] rounded-xl transition duration-300 shadow-[0_4px_20px_rgba(52,211,153,0.2)]"
        >
          <PlusCircle size={16} />
          Create Project
        </Link>
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {statCards.map((item) => {
          ...
          return (
            <div
              key={item.title}
              className="rounded-[30px] border border-[var(--border)] bg-[var(--surface)] backdrop-blur-md p-6 transition-all duration-300 hover:border-[var(--primary)]/40 hover:shadow-[0_0_30px_rgba(52,211,153,0.08)]"
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-[var(--primary)]/20 bg-[var(--primary)]/10 text-[var(--primary)]">
                  <Icon size={20} />
                </div>
              </div>
              <p className="mt-4 text-xs text-[var(--muted)]">{item.description}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="rounded-[30px] border border-[var(--border)] bg-[var(--surface)] backdrop-blur-md p-6 md:p-8 transition-all duration-300 hover:border-[var(--primary)]/40">
        ...
        {recentProjects.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-[var(--border)] rounded-2xl bg-[var(--surface-strong)]">
              ...
              <tbody className="divide-y divide-[var(--border)] text-sm">
                {recentProjects.map((project) => {
                  ...
                  return (
                    ...
                    >
                      <td className="p-4 font-bold text-[var(--text)] max-w-50 truncate">
                        {project.title}
                      </td>
                      <td className="p-4">
                        <span className="bg-[var(--surface)] px-2.5 py-1 rounded-md text-xs font-medium text-[var(--muted)] border border-[var(--border)]">
                          {project.category}
                        </span>
                      </td>
                      ...
                      <td className="p-4 text-right">
                        <Link
                          ...
                          className="inline-flex items-center gap-1 text-xs text-[var(--primary)] hover:bg-[var(--primary)]/10 border border-[var(--primary)]/20 px-3 py-1.5 rounded-xl font-bold transition-all"
                        >
