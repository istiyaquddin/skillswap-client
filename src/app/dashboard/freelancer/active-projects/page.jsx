    ...
    const fetchProjects = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${apiUrl}/api/freelancer-projects?email=${session.user.email}`,
          {
            method: "GET",
            headers: {
      ...
      <div className="glass-panel rounded-[2.5rem] p-6 md:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-sm text-[var(--muted)]">
            Manage active project contracts, submit deliverables, and track
            completions.
          </p>
        </div>
        ...
        {activeProjects.length === 0 ? (
          <div className="glass-panel rounded-[2rem] py-12 text-center">
            <p className="text-sm text-[var(--muted)] italic">
              No active projects found. After your proposals are accepted, they
              will appear here.
            </p>
          </div>
        ) : (
          <div className="grid gap-4">
            {activeProjects.map((task) => (
              <div
                key={task._id}
                className="glass-panel rounded-[2rem] p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition hover:border-amber-500/40 hover:scale-[1.01]"
              >
                <div className="space-y-2">
                  <h3 className="font-bold text-base text-[var(--text)]">
                    {task.title}
                  </h3>
                  <p className="text-xs text-[var(--muted)] line-clamp-1 max-w-xl">
                    {task.description}
                  </p>
        ...
        {completedProjects.length === 0 ? (
          ...
          <div className="grid gap-4">
            {completedProjects.map((task) => (
              <div
                key={task._id}
                className="glass-panel rounded-[2rem] p-6 relative space-y-4 transition hover:border-emerald-500/30"
              >
                ...
                <div className="space-y-1.5">
                  <h3 className="font-bold text-base pr-20 text-[var(--text)]">
                    {task.title}
                  </h3>
